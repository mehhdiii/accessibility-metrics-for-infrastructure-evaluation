...
int main(int argc, char *argv[])
{
    if (argc < 8)
    {
        std::cerr << "Not enough arguments - " << argv[0] << " <input pcd> <output pcd> <mysqlhost> <port> <username> <password> <dbname>" << std::endl;
        return 1;
    }
    ...
    unsigned int port = std::stoi(argv[4]);

    int point_cloud_id = std::stoi(argv[1]);
    mainCloud = loadPointCloudFromMySQL(
        argv[3], // db variables
        port,
        argv[5],
        argv[6],
    ...
    if (detectedStairs.size() > 0)
    {
        for (int stairCoeffIdx = 0; stairCoeffIdx < detectedStairs.size(); stairCoeffIdx++)
        {
            ...
            std::cout << std::endl
                      << "Step depth:   " << round(1000 * sqrt(pow(stairCoefficients.dir[0], 2) + pow(stairCoefficients.dir[1], 2))) << std::endl;
            ...
            std::cout << "Amount of stair parts: " << stairCoefficients.size() << std::endl
                      << std::endl;
            ...
            float step_depth = round(1000 * sqrt(pow(stairCoefficients.dir[0], 2) + pow(stairCoefficients.dir[1], 2)));
            float step_height = round(1000 * stairCoefficients.dir[2]);
            float step_width = round(1000 * stairCoefficients.width);
            float slope_deg = round(100 * steigung / M_PI * 180);
            float stair_angle_deg = round(100 * atan2(stairCoefficients.dir[1], stairCoefficients.dir[0]) / M_PI * 180);
            int stairParts = stairCoefficients.size();
            float distX = round(1000 * (stairCoefficients.pos[0]));
            float distY = round(1000 * stairCoefficients.pos[1]);
            float distPar = round(1000 * sepDist[0]);
            float distOrt = round(1000 * sepDist[1]);
            float anchorPoint = stairCoefficients.anchPoint;
            int risers = stairRisers.size();
            int treads = stairTreads.size();

            unsigned int port = std::stoi(argv[4]);

            saveStairsToDatabase(
                argv[3], // db variables
                port,
                argv[5],
                argv[6],
    ...
    if (detectedStairs.size() > 0)
    {
        ...
        {
            // --- Visualization ---
            pcl::visualization::PCLVisualizer::Ptr viewer(new pcl::visualization::PCLVisualizer("Stair Viewer"));
            viewer->setBackgroundColor(0, 0, 0); // black background
            viewer->addPointCloud<pcl::PointXYZRGB>(resultCloud.makeShared(), "stairs");
            viewer->addCoordinateSystem(0.1); // axes, 0.1 meters
            viewer->initCameraParameters();

            // Keep the viewer open until the user closes it
            while (!viewer->wasStopped())
            {
                viewer->spinOnce(100); // update every 100 ms
